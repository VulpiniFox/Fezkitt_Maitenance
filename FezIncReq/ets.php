	...
	function parse($ptype, &$i, &$line, $ncontent, $content)
	{
		...
		for (; $i < $ncontent; ++$i)
		{
			...
			if ($mode == _ETS_DATA)
			{
				...
				if ($c0 == '{')
				{
					...
					} elseif ($c1 == 'i' && $c2 == 'n' && $c3 == 'c' && $c4 == 'l' && $c5 == 'u' && $c6 == 'd' && $c7 == 'e' & $content{$i + 8} == ':')
					{
						...
						{
							$return = NULL;
							# fix for eval having problems imploding the path
							$var = implode('', $container_name);
							eval("\$return=(string)\"$var\";");
							if (isset($return))
							{
			...
			} elseif ($mode == _ETS_VALUE)
			{
				...
				{
					if ($nvalue == 0 && $quotetype == 0)
					{ // no value without quote
	...
	function read_container($container, $parse)
	{
		...
		if ($parse != _ETS_TEXT)
		{
			...
			} else
			{
				...
				$this->containers[$container] = TRUE;
				# fix for error
				# DEFAULT WARNING [#2048] Only variables should be passed by reference
				# Error encountered on line 1471 in file /home/kandykid/public_html/e-r_increq/ets.php
				$i = 0;
				$line = 1;
